...
import { isAos, NowDate } from "../utils"
...
export const useUserLocationHook = () => {
    ...
    const checkLocationPermission = async () => {
        try {
            if (isAos) {
                ...
                const statuses = await checkMultiple([PERMISSIONS.IOS.LOCATION_WHEN_IN_USE, PERMISSIONS.IOS.LOCATION_ALWAYS])
                return statuses[PERMISSIONS.IOS.LOCATION_WHEN_IN_USE] === "granted" || statuses[PERMISSIONS.IOS.LOCATION_ALWAYS] === "granted"
            }
        } catch (err) {
            console.warn(err)
    ...
    const addUserAddress = async (addedAddress: MY_ADDRSS) => {
        ...
        const newMyAddress = myAddress.filter(({ location }) => location !== addedAddress.location)
    ...
    return { requestLocationPermission, checkLocationPermission, getUserLocation, addUserAddress, removeUserAddress }
}
